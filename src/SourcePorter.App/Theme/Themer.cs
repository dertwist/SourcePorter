...
/// <summary>
/// Source 2 Viewer (ValveResourceFormat GUI) theme, ported 1:1 from
/// <c>GUI/Utils/Themer.cs</c>. The colour palette, the per-control styling in
/// <see cref="ThemeControlInternal"/>, the menu/toolbar renderer, and the use of
/// .NET 9's <see cref="Application.SetColorMode"/> all match the original.
///
/// Branches for VRF-specific custom controls (ExplorerControl, CodeTextBox,
/// BetterListView, HSVSlider, the custom control box, …) and the SkiaSharp SVG
/// helpers are omitted because SourcePorter does not ship those types; every
/// standard WinForms control is themed identically to the original.
/// </summary>
public static class Themer
{
    ...
    /// <summary>
    /// Stand-in for VRF's <c>Settings.Config.Theme</c>. SourcePorter mirrors the
    /// Source 2 Viewer default look, so this defaults to <see cref="AppTheme.Dark"/>.
    /// </summary>
    public static AppTheme ConfiguredTheme { get; set; } = AppTheme.Dark;
    ...
    public static void InitializeTheme()
    {
        ...
        if (theme == AppTheme.System || !Enum.IsDefined(theme))
        {
    ...
    // ---- SVG icon rendering (ported from VRF Themer.cs SvgToSkiaBitmap/GetSvgBitmap) ----
    ...
    /// <summary>Loads an embedded Source 2 Viewer icon (<c>Icons/{name}.svg</c>) as an SKBitmap.</summary>
    public static SKBitmap GetSvgBitmap(string iconName, int width, int height)
    {
        using var svgResource = Assembly.GetExecutingAssembly()
            .GetManifestResourceStream($"SourcePorter.App.Icons.{iconName}.svg")
            ?? throw new InvalidOperationException($"Embedded icon not found: {iconName}");
        using var svg = new SKSvg();
        svg.Load(svgResource);
        return SvgToSkiaBitmap(svg, width, height);
    }

    /// <summary>Renders an embedded SVG icon to a square <see cref="Bitmap"/> at <paramref name="size"/> px.</summary>
    public static Bitmap GetIcon(string iconName, int size)
    {
        using var skBitmap = GetSvgBitmap(iconName, size, size);
        using var image = SKImage.FromBitmap(skBitmap);
        using var data = image.Encode(SKEncodedImageFormat.Png, 100);
        using var ms = new MemoryStream(data.ToArray());
        using var fromStream = new Bitmap(ms);
        return new Bitmap(fromStream); // independent copy so the stream can be disposed
    }
}

/// <summary>A <see cref="Panel"/> that opts out of <see cref="Themer"/>'s automatic styling.</summary>
public sealed class UnstyledPanel : Panel { }

// Custom Renderers for Menus and ToolBars (ported 1:1 from VRF).
public class DarkToolStripRenderer(ProfessionalColorTable table) : ToolStripProfessionalRenderer(table)
{
    ...
    protected override void OnRenderSeparator(ToolStripSeparatorRenderEventArgs e)
    {
        ...
        if (e.Vertical)
        {
            ...
            g.DrawLine(separatorPen, centerX, bounds.Top, centerX, bounds.Bottom);
        }
        else
        {
            var centerY = bounds.Height / 2;
            g.DrawLine(separatorPen, bounds.Left, centerY, bounds.Right, centerY);
        }
    }
